...
//        $content = str_replace("  \n"," \n", $content);
    ...
    public function post_options($post = NULL)
    {
        ...
        // NSFW
        if(PeepSo::is_dev_mode('nsfw') && PeepSo::get_option_new('nsfw') && PeepSo::can_nsfw($post->ID)) {
            $options['nsfw'] = array(
    ...
    public function post_options_add_toggle_comments($options, $post) {
        if(apply_filters('peepso_can_disable_comments', PeepSo::is_admin(), $post->ID)) {
    ...
    public function set_comments_status(PeepSoAjaxResponse $resp) {
        ...
        if(!apply_filters('peepso_can_disable_comments', PeepSo::is_admin(), $post_id))
        {
            $resp->error(__('You do not have permission to do that', 'peepso-core'));
            $resp->success(FALSE);
            return FALSE;
